...
function checkData(data) {
  ...
  const myErr =new Error("Об'єкт пустий");
...
function parseJson(jsonStr) {
  ...
  // Якщо рядок має невірний формат, виникне помилка, яку ми обробляємо у блоку catch.
    ...
    // Якщо виникла помилка, повертаємо її повідомлення.
    return new Error("Невірний формат JSON");
  }
}
...
function getAge(age) {
  ...
  if (age < 0) {
    ...
    const myErr = new Error("Вік не може бути менше 0!");
...
function validateUser(user) {
  ...
  // Виводимо повідомлення про помилку та причину помилки.
    ...
    console.log(`${error.message} ${JSON.stringify(error.cause)}`);
}
}
